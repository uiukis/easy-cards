    ...
    <section className="relative py-20 sm:py-28">
      <div className="mx-auto max-w-6xl px-5 sm:px-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {founders.map((f, i) => {
            const Wrapper = f.instagram ? motion.a : motion.div;
            return (
              <Wrapper
                key={f.id}
                {...(f.instagram
                  ? { href: f.instagram, target: "_blank", rel: "noopener noreferrer" }
                  : {})}
              ...
              >
                {f.image_url ? (
                  ...
                  <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full border-2 border-ink/10 bg-surface-alt font-display text-xl text-orange-deep">
